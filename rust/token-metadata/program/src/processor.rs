use {
    ...
    agsol_token_vault::{error::VaultError, state::VaultState},
};
...
pub fn process_mint_new_edition_from_master_edition_via_vault_proxy<'a>(
    ...
    if vault_data[0] != agsol_token_vault::state::Key::VaultV1 as u8 {
        return Err(VaultError::DataTypeMismatch.into());
    }

    if safety_deposit_data[0] != agsol_token_vault::state::Key::SafetyDepositBoxV1 as u8 {
        return Err(VaultError::DataTypeMismatch.into());
    }
    ...
    let seeds = &[
        agsol_token_vault::state::PREFIX.as_bytes(),
